...
interface PolitiqueDurabiliteProps {
  subTab: string;
}

export const PolitiqueDurabilite: React.FC<PolitiqueDurabiliteProps> = ({ subTab }) => {
  ...
  useEffect(() => {
    console.log('PolitiqueDurabilite - subTab:', subTab);
    console.log('PolitiqueDurabilite - currentOrganization:', currentOrganization);
    
    if (!currentOrganization || !subTab) {
      console.warn('Paramètres manquants:', { currentOrganization, subTab });
      setLoading(false);
      return;
    }
  ...
  const fetchContentModules = async () => {
    if (!currentOrganization || !subTab) {
      console.warn('fetchContentModules - paramètres manquants:', { currentOrganization, subTab });
      return;
    }
  ...
  const loadModuleImages = async () => {
    if (!currentOrganization || !subTab) {
      console.warn('loadModuleImages - paramètres manquants:', { currentOrganization, subTab });
      return;
    }

    const moduleKey = `durabilite_${subTab}`;
    const images = await getModuleImages('durabilite', subTab);
    setModuleImages({ [moduleKey]: images });
  };

  // CORRECTION: Ajout de vérifications dans sanitizeForStorage
  const sanitizeForStorage = (str: string): string => {
    if (!str || typeof str !== 'string') {
      console.warn('sanitizeForStorage - str invalide:', str);
      return 'unknown';
    }
    return str
      .toLowerCase()
  ...
  const handleEditModule = () => {
    if (!currentOrganization || !subTab) {
      console.warn('handleEditModule - paramètres manquants:', { currentOrganization, subTab });
      return;
    }

    const moduleKey = `durabilite_${subTab}`;
    const existingModule = contentModules.find(m => 
      m.module_type === 'durabilite' && m.sub_type === subTab
    );
  ...
  const handleSaveModule = async () => {
    if (!editingContent.module_type || !currentOrganization || !subTab) {
      toast.error('Paramètres manquants pour la sauvegarde');
      return;
    }
  ...
  // CORRECTION: Vérifications strictes pour handleImageUpload
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!subTab) {
      toast.error('Type de module non défini');
      return;
    }

    if (!editingModule) {
      toast.error('Module non en édition');
      return;
    }
    ...
    try {
      ...
      const sanitizedOrganization = sanitizeForStorage(currentOrganization);
      const sanitizedModuleType = sanitizeForStorage('durabilite');
  ...
  const handleRemoveImage = async (imageUrl: string, index: number) => {
    if (!editingModule || !moduleImages[editingModule] || !currentOrganization || !subTab) {
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <input
                    ...
                    className="hidden"
                    id={`image-upload-${subTab || 'default'}`}
                  />
                  <label
                    htmlFor={`image-upload-${subTab || 'default'}`}
