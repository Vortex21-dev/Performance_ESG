...
export const RoadMapDD: React.FC = () => {
  ...
  const fetchContentModules = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq('module_type', 'outils_dirigeants');
  ...
  const loadModuleImages = async () => {
    ...
    const moduleKey = 'outils_dirigeants';
    const images = await getModuleImages('outils_dirigeants');
  ...
  const handleEditModule = () => {
    ...
    const existingModule = contentModules.find(m => 
      m.module_type === 'outils_dirigeants' && !m.sub_type
    );
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const sanitizedOrganization = sanitizeForStorage(currentOrganization);
      const sanitizedModuleType = sanitizeForStorage('outils_dirigeants');
      
      const filePath = `${sanitizedOrganization}/${sanitizedModuleType}/${fileName}`;
