...
interface StrategieProps {
  subTab: 'vision' | 'strategie';
}

export const Strategie: React.FC<StrategieProps> = ({ subTab }) => {
  ...
  const loadModuleImages = async () => {
    if (!currentOrganization) return;
    const moduleKey = `strategie_${subTab}`;
    const images = await getModuleImages('strategie', subTab);
  ...
  const getModuleImages = async (moduleType: string, subType?: string): Promise<string[]> => {
    ...
    try {
      ...
      const { data: files, error } = await supabase.storage.from('ImagesManagement').list(folderPath);
  ...
  const handleEditModule = () => {
    const moduleKey = `strategie_${subTab}`;
    const existing = contentModules.find(m => m.module_type === 'strategie' && m.sub_type === subTab);
    if (existing) {
  ...
  const handleSaveModule = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('content_modules')
        .upsert(moduleData, { onConflict: 'organization_name,module_type,sub_type' })
        .select()
        .single();
      ...
      setContentModules(prev => [
        ...prev.filter(m => !(m.module_type === 'strategie' && m.sub_type === subTab)),
        data
      ]);
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !currentOrganization || !editingModule) {
      toast.error('Organisation ou module non défini.');
      return;
    }
    ...
    try {
      ...
      const sanitizedOrganization = sanitizeForStorage(currentOrganization);
      const sanitizedModuleType = sanitizeForStorage('strategie');
  ...
  const getModuleContent = () => {
    return contentModules.find(m => m.module_type === 'strategie' && m.sub_type === subTab);
  };
          ...
          <div className="space-y-6">
            {!content?.content && images.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">Ce module n'a pas encore été configuré.</p>
              </div>
            ) : (
              <>
                {content?.content && (
